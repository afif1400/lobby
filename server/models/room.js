...
const removeUserFromRoom = (user) => {
  ...
  if (team_name) {
    ...
    const newTeam = rooms[room_id].teams[team_name].filter(
      (ele) => ele !== userName
    );
    ...
    const newBench = rooms[room_id].state.bench.filter(
      (ele) => ele !== userName
    );
...
const joinTeam = (user, team_name) => {
  ...
  // remove from bench
  const newBench = rooms[user.room_id].state.bench.filter(
    (ele) => ele !== userName
  );
...
const closeRoom = (user, forceCloseRoom = false) => {
  ...
  const allMembers = rooms[room_id].state.bench;
  // from all teams
  Object.keys(rooms[room_id].teams).forEach((team_name) => {
    rooms[room_id].teams[team_name].forEach((usr) => {
      allMembers.push(usr);
    });
  });
...
const leaveTeam = (user) => {
  ...
  const newTeam = room.teams[team_name].filter((ele) => ele !== userName);
...
const registerVotes = ({ room_id, userName, team_name, votes }) => {
  let tempVotes = votes;
  if (!room_id || !userName || !tempVotes || !team_name) {
  ...
  // valid votes only
  tempVotes = tempVotes.filter((id) => allQuestions.includes(id));
  // votes should be unique
  tempVotes = [...new Set(tempVotes)];
  // should not excede max_votes allowed
  // we will only take Min(votes.length, max_vote) votes
  if (tempVotes.length > max_vote) tempVotes = tempVotes.slice(0, max_vote);
  // note votes
  tempVotes.forEach((id) => {
  ...
  let totalRequired = 0;
  Object.keys(rooms[room_id].teams).forEach((teamName) => {
    totalRequired += rooms[room_id].teams[teamName].length;
  });
...
const atLeastPerTeam = (room_id, min_size = 1) => {
  try {
    for (const [memList] of Object.entries(rooms[room_id].teams)) {
