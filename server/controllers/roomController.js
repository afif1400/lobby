...
const createRoom = (config, { socket }) => {
  ...
  console.log(room_id);
  // updateUser doesn't return anything for now, could be changed and checked
  UserModel.updateUser({ userName: config.userName, room_id });
  socket.join(room_id);
  // created room
...
const joinTeam = ({ userName, team_name }, { socket }) => {
  ...
  UserModel.updateUser({ userName, team_name });
...
const closeRoom = ({ userName, forceCloseRoom }, { socket }) => {
  ...
  const allMembers = room_obj.returnObj;
  ...
  allMembers.forEach((member) => {
    ...
    UserModel.updateUser({ userName: member, room_id: '', team_name: '' });
  });
...
const doVeto = async (quesIds, room_id, count, socket) =>
  new Promise((resolve, reject) => {
    ...
    let room_obj = RoomModel.doVeto(quesIds, room_id, count, state);
...
const startCompetition = async ({ userName }, { socket }) => {
  ...
  let room_obj = RoomModel.startCompetition(user, state);
...
const codeSubmission = async (
  ...
  if (room_check.status === 0) {
    return { err: room_check.returnObj };
  }
  const room = room_check.returnObj;
  submitCode(testcase, code, langId, (dataFromSubmitCode) => {
    ...
    if (allPass) {
      ...
      let state = 'one-pass';
      let room_obj = RoomModel.codeSubmission(
        room_id,
        state,
        team_name,
        quesId
      ...
      if (
        ...
        state = 'all-pass';
        room_obj = RoomModel.codeSubmission(room_id, state);
        socket.to(room_id).emit(COMPETITION_STOPPED, room_obj.returnObj);
        socket.emit(COMPETITION_STOPPED, room_obj.returnObj);
      }
    }
